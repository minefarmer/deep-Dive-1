...
# If I have a lot of fields in the namedtuple--the code is not as clean any more

Stock = namedtuple('Stock", 'symbol year month day open high low close')

Stock._fields -> 'symbol', 'year', 'month', 'day', 'open', 'high', 'low', 'close'

I can then create a new named tuple by "extending" the _fields tuple

new_fields = Stock._fields + ('previous close', )

StockExt = namedtuple('StockExt', new_fields)


We can also easily use an existing Stock instance to create a new StockExit instance with the same common values, adding in our new previous_close value:
...
djia_ext = Stockxt(*djia, 26_000)

or

djia_ext = StockExt._make(djia + {26000})
