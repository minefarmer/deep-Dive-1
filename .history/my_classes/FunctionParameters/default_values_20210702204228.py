...
Solution Pattern = need to show current time it was executed
    ...
    otherwise, use what the caller specified for datetime
        ...
        from datetime import datetime         > recall that this is equivalent to:
                                            /   if not dt:
        def log(msg, *, dt=None):          /        dt = datetime.utcnow()
            dt = dt or datetime.i=utcnow()<
            print ('{0}: {1}'.format(dt, msg))

    ##  In general, always beware of using a mut
