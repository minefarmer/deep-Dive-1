...
When Python encounters a function definition at compile-time
    it will scan for any labels (variables) that have values assigned to them (anywhere in a function)
...
a = 10
                ...
                /a is referenced only in entire function at complie time -> a non-local
def func1():  /
    print(a) <

              / assignment
def func2(): /  at compile time -> a local
    a = 100 /

def func():     / assignment
    global a   /  at compile time -> a global
    a = 100  </   (because we told Python a was global)

def func4():    / assignment
    global a   /  at compile time -> a local
    a = 100   /
                -> when we call func4()
                print(a)  # results in a runtime error

                bdcause a is local, and we are referencing it before  we have assigned a value to it
"""